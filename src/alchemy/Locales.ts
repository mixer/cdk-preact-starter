...
export class Locales extends EventEmitter {
    ...
    constructor() {
        ...
        const settings = Mixer.display.getSettings();
        if (settings) {
            this.loadLocale(settings.language);
        }

        Mixer.display.on('settings', ({ language }) => this.loadLocale(language));
    }
     ...
     */
    public loadLocale(locale: string): void {
        ...
        let bestIndex = -1;
        let bestCommon = 0;
        Mixer.locales.map(splitLocale).forEach((locale, i) => {
            let common = 0;
            for (; common < locale.length && common < current.length; common++) {
                if (locale[i] !== current[i]) {
                    break;
                }
            }
