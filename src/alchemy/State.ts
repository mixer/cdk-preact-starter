...
export class State extends EventEmitter {
  ...
  private bindSceneListeners() {
    Mixer.socket.on(
      'onSceneCreate',
      guard(({ scenes }) => {
        scenes.forEach(s => {
          assert(
            !this.scenes[s.sceneID],
            `Tried to create scene in "${s.sceneID}", but it already exists`,
          );
    ...
    Mixer.socket.on(
      ...
      guard(({ scenes }) => {
        scenes.forEach(s => {
          assert(
            this.scenes[s.sceneID],
            `Tried to update scene "${s.sceneID}", but it didn't exist`,
          );
    ...
    Mixer.socket.on(
      ...
      guard(packet => {
        assert(
          this.scenes[packet.sceneID],
          `Tried to update scene "${packet.sceneID}", but it didn't exist`,
        );
    ...
    Mixer.socket.on(
      ...
      guard(data => {
        assert(
          this.scenes[data.sceneID],
          `Tried to create controls in "${data.sceneID}", but it didn't exist`,
        );
    ...
    Mixer.socket.on(
      ...
      guard(data => {
        assert(
          this.scenes[data.sceneID],
          `Tried to update controls in "${data.sceneID}", but it didn't exist`,
        );
    ...
    Mixer.socket.on(
      ...
      guard(data => {
        assert(
          this.scenes[data.sceneID],
          `Tried to delete controls in "${data.sceneID}", but it didn't exist`,
        );
  ...
  private bindGroupsListeners() {
    ...
    Mixer.socket.on(
      ...
      guard(({ groups }) => {
        groups.forEach(g => {
          assert(
            !this.groups[g.groupID],
            `Tried to create group "${g.groupID}", but it already exists`,
          );
          assert(
    ...
    Mixer.socket.on(
      ...
      guard(({ groups }) => {
        groups.forEach(g => {
          assert(
            this.groups[g.groupID],
            `Tried to update group "${g.groupID}", but it didn't exist`,
          );
          assert(
            this.scenes[g.sceneID],
            `Tried to assign group to "${g.sceneID}", but it didn't exist`,
    ...
    Mixer.socket.on(
      ...
      guard(packet => {
        assert(
          this.groups[packet.groupID],
          `Tried delete group "${packet.groupID}", but it doesn't exist`,
        );
...
/**
 * A Resource is the base class for Interactive models. Scenes, Controls,
 * Groups, and Participants are all Rersources. You can read properties
 * on these resources, but as a participant you cannot update them.
 */
...
export class Group<T extends Mixer.IGroup = Mixer.IGroup> extends Resource<T> {
   ...
   */
  public get<R>(prop: string, defaultValue: R): R {
    const props: any = this.props;
    return props[prop] === undefined ? defaultValue : props[prop];
  }
